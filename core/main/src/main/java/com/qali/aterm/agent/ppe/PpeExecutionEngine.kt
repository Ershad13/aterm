        ...
        if (continuationResponse.functionCalls.isNotEmpty()) {
            ...
            if (shouldContinue) {
                ...
                val promptText = when {
                    ...
                    functionCall.name == "write_file" -> "Good! Keep creating files. You need: package.json (if missing), index.html, style.css, and client.js. Use write_file to create each one. IMPORTANT: Check the code relativeness information in the tool result. Only use imports/exports that exist in related files. Continue until all files are created."
                    ...
                    if (continueResponse != null) {
                        ...
                        if (continueResponse.functionCalls.isEmpty() && continueResponse.text.isNotEmpty() && (functionCall.name == "write_file" || functionCall.name == "shell" || functionCall.name == "ls" || functionCall.name == "read")) {
                            ...
                            val finalPromptText = when (functionCall.name) {
                                "write_file" -> "You must continue creating files. The project is not complete yet. Create the remaining files now: package.json (if not created), HTML file (index.html), CSS file (style.css or styles.css), and any client-side JavaScript files. Use write_file to create each file. IMPORTANT: Check the code relativeness information from previous write_file results. Only use imports/exports that exist in related files."
